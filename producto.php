<!-- backend -->
<?php
	if (isset($_GET['informacionProductoTop'])) {
		$id = $_GET['informacionProductoTop'];
		require("admin/conexion.php");
		$queryProducto = "SELECT * FROM top WHERE id='$id'";
		$queryProductoResult = mysqli_query($conexion, $queryProducto);
		$row = mysqli_fetch_array($queryProductoResult);
		mysqli_close($conexion);
		$imgRoute = "../productos/top-".$id.".".$row['img'];
	}
	if (isset($_GET['informacionProducto'])) {
		$id = $_GET['informacionProducto'];
		require("admin/conexion.php");
		$queryProducto = "SELECT * FROM categorias WHERE id='$id'";
		$queryProductoResult = mysqli_query($conexion, $queryProducto);
		$row = mysqli_fetch_array($queryProductoResult);
			...
			<div class="container">
				...
				<div id="responsive-nav">
					...
					<ul class="main-nav nav navbar-nav">
						<li class="active"><a href="#"><?= $row['nombre'] ?></a></li>
				...
				<div class="row">
					...
					<div class="col-md-6">
						<div class="product-details">
							<h2 class="product-name">Sabemos que te interesa estre producto, dejanos tu dato para contactarte y te haremos saber el precio.</h2>
							<br>
							<h5><?= $row['nombre'] ?></h5>
							<div id="review-form">
								<form action="mensajes.php" method="post" class="review-form">
									<input name="contacto" class="input" type="text" placeholder="Dato De Contacto Ej: WhatsApp o Correo Electronico" value="">
									<input name="motivo" class="input" type="hidden" value="Pregunta por : <?= $row['nombre'] ?>">
									<textarea name="comentario" class="input" placeholder="Comentario (Opcional)"></textarea>
									<button type="submit" name="pregunta" class="primary-btn">Preguntar</button>
								</form>
							</div>
						</div>
					</div>
